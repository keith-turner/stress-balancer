...
public abstract class GroupBalancer extends TabletBalancer {
  ...
  public static class Location {
    ...
    private TServerInstance tserverInstance;

    public Location() {
      tserverInstance = null;
    }
  ...
  private static class TserverGroupInfo {
    ...
    private Map<String, MutableInt> initialCounts = new HashMap<>();
    private Map<String, Integer> extraCounts = new HashMap<>();
    private Map<String, Integer> expectedDeficits = new HashMap<>();
  ...
  private static class Moves {
    ...
    private Table<TServerInstance,String,List<Move>> moves = HashBasedTable.create();
  ...
  class MetadataLocationProvider implements Iterable<Pair<KeyExtent,Location>> {
    ...
    public Iterator<Pair<KeyExtent,Location>> iterator() {
      try {
        SystemCredentials creds = SystemCredentials.get(configuration.getInstance());
        Connector conn = new ZooKeeperInstance(configuration.getInstance().getInstanceName(), configuration.getSiteConfiguration().get(
            Property.INSTANCE_ZK_HOST)).getConnector(creds.getPrincipal(), creds.getToken());
